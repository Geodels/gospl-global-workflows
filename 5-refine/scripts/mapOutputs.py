...
class mapOutputs:
    def __init__(self, path=None, filename=None, step=None, uplift=True, bio=False):
    ...
    def getData(self, step):
        ...
        if nelev[self.indices].ndim == 2:
            ...
            if not self.bio:
                self.rain = (
                    np.sum(self.weights * nrain[self.indices][:, :], axis=1)
                    / self.sumwght
                )
                self.erodep = (
    ...
    def buildLonLatMesh(self, res=0.1, nghb=3, box=None):
        ...
        if not self.bio:
            ...
            if self.lookuplift and self.uplift is not None:
                uplifti = (
                    np.sum(self.wghts * self.uplift[self.ids], axis=1) * self.denum
                )
    ...
    def exportNetCDF(self, ncfile):
        ...
        if self.lookuplift and not self.bio:
